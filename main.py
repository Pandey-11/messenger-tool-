...
def main():
    logo()
    print("[1] Login using fbstate.json file")
    ...
    if choice == "1":
        if not os.path.exists("fbstate.json"):
            print("[x] fbstate.json file not found! Put it in same folder.")
            return
        try:
            with open("fbstate.json", "r", encoding="utf-8") as f:
                fbstate = json.load(f)
            cookies = {}
            for item in fbstate:
                cookies[item["key"]] = item["value"]
            session.cookies.update(cookies)
            print("[✓] fbstate.json loaded successfully!")
        except Exception as e:
            print(f"[x] Failed to load fbstate.json: {e}")
            return
